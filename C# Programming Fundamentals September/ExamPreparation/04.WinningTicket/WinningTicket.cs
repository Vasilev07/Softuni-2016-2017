            ...
            foreach (var ticket in tickets)
            {
                ...
                foreach (var winningSymbol in winningSymbols)
                {
                    ...
                    if (leftMatch.Success)
                    {
                        ...
                        {
                            winningTicket = true;
                            var leftMatchValue = leftMatch.Value.Length;
                            var rightMatchValue = rightMatch.Value.Length;
                            var jackpot = leftMatchValue == 10 && rightMatchValue == 10
                                ? " Jackpot!" : string.Empty;
                            Console.WriteLine($"ticket \"{ticket}\" - {Math.Min(leftMatchValue,rightMatchValue)}{winningSymbol.Trim('\\')}{jackpot}");
                            break;
                        }
                    }
                }
